...
class Jonny_WINDOW:
    def __init__(self):
        ...
        self.label.set_text("Порт VR Glass:")
        ...
        self.label3.set_text("Осталось времени: "+ str(self.timer))
        self.label3.set_line_wrap(True)
        self.vbox.pack_start(self.label3, 0, True, 0)
    ...
    def rend_timer(self):
        while(not self.Exit):
            if(self.INIT):
                self.timer=self.video.get_time()
                self.label3.set_text("Осталось времени: "+ str(self.timer))
            time.sleep(0.9)
