...
num_spedometr_texture=RTCvideo.texture_block()
speedR=Speedometr([0.57, 0.5, 0.0], 0.05)
speedL=Speedometr([-0.57, 0.5, 0.0], 0.05)

speedNR=Number_Spedometr([0.60-4*0.05-0.1, -0.15 ,-1.0], 0.05)
speedNL=Number_Spedometr([-0.60, -0.15 ,-1.0], 0.05)
...
AUDIO_CAPS="audio/x-raw,clock-rate=8000,format=S16LE"
#AUDIO_CAPS="audio/x-raw,rate=48000,channels=1,depth=16"
...
def initGL():
    ...
    num_spedometr_texture.set_image("numbers_v2.png")
    ...
    if(num_spedometr_texture.pixel_x*num_spedometr_texture.pixel_x>0):
        num_spedometr_texture.gen_texture()
        if( num_spedometr_texture.texture ==0):        
            print("ERROR: Gen texture\n")
...
def draw_arrow(texture0):
    ...
    glVertex3f(-20.0, texture0.vertexcoord1[1], -100.0)
    glVertex3f(-20.0, texture0.vertexcoord2[1], 100.0)
    glVertex3f(-20.0+H, texture0.vertexcoord3[1], 100.0 )
    glVertex3f(-20.0+H, texture0.vertexcoord4[1], -100.0)
    glEnd()

    glBegin(GL_QUADS)
    glColor4f(1.0, 1.0, 0.0, 1.0)
    glVertex3f(20.0-H, texture0.vertexcoord1[1], -100.0)
    glVertex3f(20.0-H, texture0.vertexcoord2[1], 100.0)
    glVertex3f(20.0, texture0.vertexcoord3[1], 100.0 )
    glVertex3f(20.0, texture0.vertexcoord4[1], -100.0)
    glEnd()
...
def draw_callback0(GST_object, width, height, texture):
    ...
    speedNR.set_motor_power(temp)
    speedNL.set_motor_power(temp)
    ...
    speedNR.draw_num_speedometr(num_spedometr_texture.texture)
    speedNL.draw_num_speedometr(num_spedometr_texture.texture)
    ...
    gluLookAt(0.0, -7, 0.0, math.sin(ang[0]/180*math.pi),-7+math.tan((180-ang[1])/180*math.pi), -math.cos(ang[0]/180*math.pi),0,1,0)
...
################  SOURCE  ##################################        
        ...
        self.rtcpsink0.set_property('host', self.DEST)
...
###################### SECOND ################################
        ...
        self.rtcpsink1.set_property('host', self.DEST)
...
################## AUDIO #######################################
        self.audio_rtpbin = Gst.ElementFactory.make('rtpbin', 'audio_rtpbin')
        if not self.audio_rtpbin:
            print("ERROR: Could not create audio_rtpbin.")
            sys.exit(1)
        
        self.pulsesrc0 = Gst.ElementFactory.make('pulsesrc', "pulsesrc0")
        if not self.pulsesrc0:
        ...
        if not self.audio_capsfilter:
            print("ERROR: Could not create audio_capsfilter.")
            sys.exit(1)
            
        self.audio_capsfilter.set_property("caps", audio_caps)

        self.speexenc0 = Gst.ElementFactory.make('speexenc', "speexenc0")
        if not self.speexenc0:
            print("ERROR: Could not create speexenc0.")
            sys.exit(1)

        self.rtpspeexpay0 = Gst.ElementFactory.make('rtpspeexpay', "rtpspeexpay0")
        if not self.rtpspeexpay0:
            print("ERROR: Could not create rtpspeexpay0.")
            sys.exit(1)

        self.audpsink_rtpout = Gst.ElementFactory.make("udpsink", "audpsink_rtpout")
        self.audpsink_rtpout.set_property('host', self.DEST)
        self.audpsink_rtpout.set_property('port', 7000)
        

        #self.audpsink_rtpout.set_property('caps', audio_caps)

        self.audpsink_rtcpout = Gst.ElementFactory.make("udpsink", "audpsink_rtcpout")
        self.audpsink_rtcpout.set_property('host', self.DEST)
        self.audpsink_rtcpout.set_property('port', 7001)
        self.audpsink_rtcpout.set_property('sync', False)
        self.audpsink_rtcpout.set_property('async', False) 

        self.audpsrc_rtcpin = Gst.ElementFactory.make("udpsrc", "audpsrc_rtcpin")
        self.audpsrc_rtcpin.set_property('port', 7005)
...
##################################################################        
        ...
        self.player.add(self.audio_rtpbin)
        self.player.add(self.pulsesrc0)
        self.player.add(self.audio_capsfilter)
        self.player.add(self.speexenc0)        
        self.player.add(self.rtpspeexpay0)
        self.player.add(self.audpsink_rtpout)
        self.player.add(self.audpsink_rtcpout)
        self.player.add(self.audpsrc_rtcpin)
...
##################### AUDIO #####################################
        self.queuea.link_pads('src', self.audio_rtpbin, 'send_rtp_sink_2')
        self.audio_rtpbin.link_pads('send_rtp_src_2', self.audpsink_rtpout, 'sink')
        self.audio_rtpbin.link_pads('send_rtcp_src_2', self.audpsink_rtcpout, 'sink')
        self.audpsrc_rtcpin.link_pads('src', self.audio_rtpbin, 'recv_rtcp_sink_2')

        
        link_ok = self.pulsesrc0.link(self.audio_capsfilter)
        if not link_ok:
            print("ERROR: Could not link pulsesrc0 with audio_capsfilter.")
            sys.exit(1)

        link_ok = self.audio_capsfilter.link(self.speexenc0)
        if not link_ok:
            print("ERROR: Could not link audio_capsfilter with speexenc0.")
            sys.exit(1)
            
        link_ok = self.speexenc0.link(self.rtpspeexpay0)
        if not link_ok:
            print("ERROR: Could not link speexenc0 with rtpspeexpay0.")
            sys.exit(1)

        link_ok = self.rtpspeexpay0.link(self.queuea)
        if not link_ok:
            print("ERROR: Could not link speexenc0 with rtpspeexpay0.")
            sys.exit(1)
