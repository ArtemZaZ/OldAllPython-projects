...
class JoyCrashException(Exception):     # Исключение поломки джойстика
    pass
class JoyNotFoundException(Exception):  # Исключение отсутствия джойстика
    pass
...
# Взято из какого-то системного файла(для всех джойстиков)
...
class Joystick(threading.Thread):
    ...
    def connect(self, path):            # Подключается к джойстику по path
        ...
        try:
            ...
            raise JoyNotFoundException("Joystick not found")     # Вызов исключения
        else:
    ...
    def read(self):
        try:
            ...
            if evbuf:                
                time0, value, type, number = struct.unpack('IhBB', evbuf)   # Распаковка прочитанных данных                    
                ...
                if type & 0x01:                                     # если 0x01, то пришедшие данные с кнопки 
                    ...
                    if button:
                        if(self.buttonStates[button]!=value):       # если предыдущее значение не равно прочитанному(сделан вызов события нажатия кнопки прямо тут для увеличение скорости)
                            if(value):                              # если кнопка была нажата
                                #print("Кнопка нажата")
                                handler = self.buttonHandler.get(button)    # берем обработчик нажатия кнопки, который мы скинули в список
                                if(handler):                                # если он существует
                                    handler.push()                          # вызвать его
                            else:                                   # если нет
                                pass
                                #print("Кнопка отжата")
                        self.buttonStates[button] = value           # присвоить значению словаря по текущей кнопке принятое значение
    ...
    def connectButton(self, buttonName, handler):   # привязывает обработчик нажатия кнопки handler к кнопке с именем buttonName
        for but in self.buttonMap:                  # по каждой кнопке из доступных кнопок
            if(but==buttonName):                    # если такая кнопка есть
                ev = RTCevent_master.Event_block()  # создать блок события
                ev.setFun(handler)                  # дать ему функцию
                self.EV.append(ev)                  # добавить его в EVENTMASTER
