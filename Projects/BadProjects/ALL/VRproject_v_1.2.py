...
def initGL():
    ...
    texture0.set_vertexcoord([-2.0, 0.0, -20.0], [-2.0, 0.0, -16.0], [2.0, 0.0, -16.0], [2.0, 0.0, -20.0])
...
def draw_arrow(texture0):
    ...
    glPushMatrix()
    glTranslate(0.0, 1.0, -18.0)
    glRotatef(90.0, 1.0, 0.0,0.0)

    
    glColor4d(198, 198, 198, 128)
    
    glutWireCylinder( 2.0 , 1.0, 30, 20)
    glPopMatrix()
...
def draw_callback0(GST_object, width, height, texture):
    ...
    glLoadIdentity ()
    gluPerspective ( 60.0, float(1280)/float (720), 1.0, 200.0 )
    ...
    gluLookAt(0.0, -5, 0.0, math.sin(ang[0]/180*math.pi),-5+math.tan((180-ang[1])/180*math.pi), -math.cos(ang[0]/180*math.pi),0,1,0)
    glTranslate(1.0*math.cos(ang[0]/180*math.pi), 0.0, 1.0*math.sin(ang[0]/180*math.pi))
